...
import { detectPhishing, type DetectPhishingOutput } from '@/ai/flows/phishing-detector-flow';
...
function AnalysisResult({ result }: { result: DetectPhishingOutput }) {
...
export default function PhishingDetectorPage() {
  ...
  const [result, setResult] = useState<DetectPhishingOutput | null>(null);
  ...
  const handleAnalyze = async (e: FormEvent) => {
    ...
    try {
      const response = await detectPhishing({ content, type: analysisType });
      setResult(response);
    } catch (err: any) {
      console.error(err);
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-headline flex items-center gap-2">
            <ScanLine />
            <span>AI Phishing Detector Engine</span>
          </CardTitle>
          <CardDescription>
            Analyze emails, SMS messages, and links for potential phishing attempts.
          </CardDescription>
        </CardHeader>
        <CardContent>
