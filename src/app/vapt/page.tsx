...
import { Loader, Send, ScanLine, GitBranch, ChevronsRight, Milestone, ClipboardCheck, Clock, Users, FileClock, FileText, Server, Target } from 'lucide-react';
    ...
    <AccordionItem value="item-1">
      <AccordionTrigger>
        <div className='flex items-center gap-2'>
            <GitBranch />
            <span className="font-headline text-lg">Our VAPT Process Explained</span>
        </div>
      </AccordionTrigger>
...
const vaptSteps: { id: string; title: string; icon: React.ElementType; fields: FormField[] }[] = [
    {
        id: 'scope',
...
export default function VaptPage() {
    ...
    const handleNext = () => {
        const currentFields = vaptSteps[currentStep].fields;
        for (const field of currentFields) {
            // A simple validation for non-checkbox fields
        ...
        setCurrentStep(prev => Math.min(prev + 1, vaptSteps.length - 1));
    };
    ...
    const handleSubmit = async () => {
        ...
        const lastStepFields = vaptSteps[vaptSteps.length - 1].fields;
    ...
    const activeStep = vaptSteps[currentStep];
    const progress = ((currentStep + 1) / vaptSteps.length) * 100;
        ...
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
            <header className='mb-6'>
                <div className='flex items-center gap-3 mb-2'>
                    ...
                    <h1 className='text-3xl font-headline font-bold'>{requestType}</h1>
                </div>
                <p className='text-muted-foreground'>A comprehensive security assessment to identify and exploit vulnerabilities in a defined scope.</p>
            </header>
            ...
            <Card>
                <CardHeader>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                             <CardTitle className="font-headline">{activeStep.title}</CardTitle>
                             <CardDescription>Step {currentStep + 1} of {vaptSteps.length}</CardDescription>
                        </div>
                    </div>
                </CardHeader>
                <CardContent>
                ...
                <CardFooter className="justify-between">
                    ...
                    {currentStep < vaptSteps.length - 1 ? (
